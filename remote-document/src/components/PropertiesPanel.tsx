...
// --- Local validation helpers (not reusable outside this component) ---
...
export default function PropertiesPanel() {
  ...
  const updateElement = useCallback(
    (changes: Partial<TemplateElement>) => {
      ...
      dispatch({
        type: 'UPDATE_ELEMENT',
        payload: { id: selectedElementId, changes },
      });
    },
  ...
  const updateProperties = useCallback(
    (propChanges: Partial<TemplateElement['properties']>) => {
      ...
      updateElement({
        properties: { ...selectedElement.properties, ...propChanges } as TemplateElement['properties'],
      });
    },
  ...
  const handleBringForward = useCallback(() => {
    ...
    const maxZ = Math.max(...template.elements.map((el) => el.z_order));
    if (selectedElement.z_order < maxZ) {
      updateElement({ z_order: selectedElement.z_order + 1 });
    }
  }, [template, selectedElement, updateElement]);

  const handleSendBackward = useCallback(() => {
    if (!selectedElement) return;
    if (selectedElement.z_order > 1) {
      updateElement({ z_order: selectedElement.z_order - 1 });
    }
  }, [selectedElement, updateElement]);
  ...
  if (!selectedElement) {
    return (
      <div style={styles.container}>
        ...
        <p style={styles.emptyMessage}>Select an element to edit its properties.</p>
      </div>
    );
  }
    ...
    <div style={styles.container}>
      ...
      <div style={styles.section}>
        ...
        <div style={styles.buttonRow}>
          <button type="button" style={styles.button} onClick={handleBringForward} aria-label="Bring forward">
            ↑ Forward
          </button>
          <button type="button" style={styles.button} onClick={handleSendBackward} aria-label="Send backward">
            ↓ Backward
          </button>
        </div>
      </div>
...
function PositionSection({
  ...
  onUpdate: (changes: Partial<TemplateElement>) => void;
}) {
  return (
    <div style={styles.section}>
...
function TypeSpecificProperties({
  ...
  element: TemplateElement;
  onUpdateProperties: (changes: Partial<TemplateElement['properties']>) => void;
}) {
  switch (element.type) {
    case 'static_text':
      return <StaticTextFields props={element.properties as StaticTextProperties} onUpdate={onUpdateProperties} />;
    case 'dynamic_text':
      return <DynamicTextFields props={element.properties as DynamicTextProperties} onUpdate={onUpdateProperties} />;
    case 'barcode':
      return <BarcodeFields props={element.properties as BarcodeProperties} onUpdate={onUpdateProperties} />;
    case 'qrcode':
      return <QRCodeFields props={element.properties as QRCodeProperties} onUpdate={onUpdateProperties} />;
    case 'image':
      return <ImageFields props={element.properties as ImageProperties} onUpdate={onUpdateProperties} />;
    case 'line':
      return <LineFields props={element.properties as LineProperties} onUpdate={onUpdateProperties} />;
    case 'box':
      return <BoxFields props={element.properties as BoxProperties} onUpdate={onUpdateProperties} />;
    case 'repeater':
      return <RepeaterFields props={element.properties as RepeaterProperties} onUpdate={onUpdateProperties} />;
    default:
      return null;
  }
}
  ...
  return (
    <div style={styles.section}>
      ...
      <DataSourceField
        ...
        staticValue={props.static_value}
    ...
    <div style={styles.section}>
      ...
      <div style={styles.fieldGroup}>
        ...
        <input
          style={styles.input}
          type="number"
          value={props.thickness_mm}
          step={0.1}
          min={0.1}
          onChange={(e) => onUpdate({ thickness_mm: parseFloat(e.target.value) || 0 })}
      ...
      <div style={styles.fieldGroup}>
        ...
        <select
          ...
          onChange={(e) => onUpdate({ orientation: e.target.value as LineOrientation })}
...
function RepeaterFields({ props, onUpdate }: { props: RepeaterProperties; onUpdate: (c: Partial<RepeaterProperties>) => void }) {
  ...
  const handleAddColumn = () => {
    const newColumn: RepeaterColumn = {
      label: `Col ${props.columns.length + 1}`,
      placeholder: `field_${props.columns.length + 1}`,
      x_offset_mm: 0,
      width_mm: 20,
      font_family: 'Arial',
      font_size_pt: 8,
    ...
    <div style={styles.section}>
      ...
      <div style={{ ...styles.fieldGroup, marginTop: '0.35rem' }}>
        ...
        <input
          style={styles.input}
          type="number"
          value={props.row_height_mm}
          min={4}
          step={0.5}
          onChange={(e) => onUpdate({ row_height_mm: parseFloat(e.target.value) || 4 })}
      ...
      <div style={{ ...styles.fieldGroup, marginTop: '0.35rem' }}>
        ...
        <input
          ...
          max={50}
          step={1}
          onChange={(e) => onUpdate({ max_rows: parseInt(e.target.value) || 1 })}
          aria-label="Max rows"
        />
      ...
      <div style={{ marginTop: '0.25rem' }}>
        <label style={styles.checkboxLabel}>
          <input
            style={styles.checkbox}
            type="checkbox"
            checked={props.show_row_lines}
            onChange={(e) => onUpdate({ show_row_lines: e.target.checked })}
            aria-label="Show row lines"
          />
          Show row lines
        </label>
      </div>
      ...
      <div style={{ marginTop: '0.5rem' }}>
        ...
        {props.columns.map((col, idx) => (
          <RepeaterColumnEditor
            key={idx}
            column={col}
            index={idx}
            canRemove={props.columns.length > 1}
            onChange={(changes) => handleColumnChange(idx, changes)}
            onRemove={() => handleRemoveColumn(idx)}
          />
        ))}
        <button
          type="button"
          style={{ ...styles.button, marginTop: '0.3rem', width: '100%' }}
...
function RepeaterColumnEditor({
  ...
  index: number;
  canRemove: boolean;
  onChange: (changes: Partial<RepeaterColumn>) => void;
  onRemove: () => void;
}) {
    ...
    <div style={{ border: '1px solid #ddd', borderRadius: '4px', padding: '0.4rem', marginBottom: '0.35rem', background: '#fafafa' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.3rem' }}>
        ...
        {canRemove && (
          <button
            type="button"
            onClick={onRemove}
            style={{ fontSize: '0.65rem', color: '#e53935', background: 'none', border: 'none', cursor: 'pointer', padding: '0.1rem 0.3rem' }}
      ...
      <div style={{ ...styles.fieldGroup, marginTop: '0.25rem' }}>
        ...
        <input
          ...
          placeholder="e.g. resi"
          aria-label={`Column ${index + 1} placeholder`}
        />
        {placeholderError && <span style={styles.error}>{placeholderError}</span>}
      </div>

      {/* x_offset_mm and width_mm */}
      <div style={{ ...styles.row, marginTop: '0.25rem' }}>
        <div style={styles.fieldGroup}>
          ...
          <input
            style={styles.input}
            type="number"
            value={column.x_offset_mm}
            min={0}
            step={0.5}
            onChange={(e) => onChange({ x_offset_mm: parseFloat(e.target.value) || 0 })}
            aria-label={`Column ${index + 1} X offset`}
          />
        </div>
        <div style={styles.fieldGroup}>
          ...
          <input
            ...
            min={1}
            step={0.5}
            onChange={(e) => onChange({ width_mm: parseFloat(e.target.value) || 1 })}
            aria-label={`Column ${index + 1} width`}
          />
        </div>
      </div>

      {/* font_size_pt and alignment */}
      <div style={{ ...styles.row, marginTop: '0.25rem' }}>
        <div style={styles.fieldGroup}>
          ...
          <input
            style={styles.input}
            type="number"
            value={column.font_size_pt}
            min={4}
            max={72}
            step={1}
            onChange={(e) => onChange({ font_size_pt: parseFloat(e.target.value) || 4 })}
            aria-label={`Column ${index + 1} font size`}
          />
        ...
        <div style={styles.fieldGroup}>
          ...
          <select
            ...
            onChange={(e) => onChange({ alignment: e.target.value as TextAlignment })}
    ...
    <>
      ...
      <div style={{ ...styles.fieldGroup, marginTop: '0.35rem' }}>
        ...
        <input
          ...
          max={72}
          step={1}
          onChange={(e) => onUpdate({ font_size_pt: parseFloat(e.target.value) || 4 })}
  ...
  return (
    <div style={{ ...styles.fieldGroup, marginTop: '0.35rem' }}>
      ...
      <select
        style={styles.select}
        value={value}
        onChange={(e) => onChange(e.target.value as TextAlignment)}
    ...
    <>
      <div style={{ ...styles.fieldGroup, marginTop: '0.35rem' }}>
        ...
        <select
          style={styles.select}
          value={dataSource}
          onChange={(e) => onUpdate({ data_source: e.target.value as DataSource })}
